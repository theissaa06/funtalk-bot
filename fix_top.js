...
const newBlock = `  if (command === 'top') {
    const period = (args[0] || 'all').toLowerCase();

    let mode = 'all';
    if (['day', 'день', 'today', 'сегодня'].includes(period)) mode = 'day';
    if (['week', 'неделя', 'weeks', 'недели'].includes(period)) mode = 'week';
    if (['month', 'месяц', 'months', 'месяца'].includes(period)) mode = 'month';
    if (['all', 'все', 'всё'].includes(period)) mode = 'all';
    ...
    function getScore(user) {
      if (mode === 'all') return user.messages || 0;
    ...
    const list = users
      .map((u, i) => (i + 1) + '. ' + usernameText(u) + ' — <b>' + u.score + '</b>')
      .join('\\n');
