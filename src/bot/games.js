...
// ── Кулдауны ─────────────────────────────────────────────────
const casinoCooldown  = new Map(); // 30 сек
const rouletteCooldown = new Map(); // 30 сек
const CASINO_CD   = 30 * 1000;
const ROULETTE_CD = 30 * 1000;

// Активные дуэли: Map<chatId, {challenger, target, amount, msgId}>
const activeDuels = new Map();
// Активные игры "угадай число": Map<chatId_userId, {number, attempts}>
...
function addCoins(userId, chatId, amount) {
  db.prepare('UPDATE users SET coins = coins + ? WHERE id = ? AND chat_id = ?').run(amount, userId, chatId);
}
...
function rouletteResult() {
  ...
  const color = ROULETTE_COLORS.red.includes(num) ? 'red' : 'black';
...
function registerGames(bot) {
  ...
  bot.command(['casino', 'слоты', 'slots'], async (ctx) => {
    if (ctx.chat.type === 'private') return ctx.reply('🎰 Казино работает только в группах!');
    ...
    if (cd > 0) return ctx.reply(`⏳ Подожди ещё <b>${cd} сек.</b> перед следующей игрой.`, { parse_mode: 'HTML' });

    const args  = ctx.message.text.split(' ').slice(1);
    const bet   = parseInt(args[0]);
    const user  = getUser(userId, chatId);

    if (!bet || bet <= 0) return ctx.reply('🎰 Укажи ставку: /casino 50');
    ...
    await ctx.reply(
      ...
      `💼 Баланс: <b>${updated.coins}</b>`,
      {
        parse_mode: 'HTML',
        ...Markup.inlineKeyboard([
          [Markup.button.callback(`🔄 Ещё раз (${bet} монет)`, `casino_again_${bet}`)],
        ]),
      }
    );
  });

  // Кнопка "Ещё раз" в казино
  bot.action(/^casino_again_(\d+)$/, async (ctx) => {
    ...
    await ctx.editMessageText(
      ...
      {
        parse_mode: 'HTML',
        ...Markup.inlineKeyboard([[Markup.button.callback(`🔄 Ещё раз (${bet} монет)`, `casino_again_${bet}`)]]),
      }
    );
  });

  // ── /roulette [ставка] [red|black|green|even|odd|число] ──────
  bot.command(['roulette', 'рулетка'], async (ctx) => {
    if (ctx.chat.type === 'private') return ctx.reply('🎡 Рулетка работает только в группах!');
  ...
  bot.command(['duel', 'дуэль'], async (ctx) => {
    ...
    if (!target || target.is_bot) {
      return ctx.reply('⚔️ Ответь на сообщение человека, которого хочешь вызвать на дуэль!\n\n/duel [сумма] — ответом на сообщение');
    }
    ...
    const args  = ctx.message.text.split(' ').slice(1);
    const bet   = parseInt(args[0]);
    if (!bet || bet < 10) return ctx.reply('⚔️ Укажи ставку (минимум 10): /duel 100 — ответом на сообщение');
    ...
    activeDuels.set(chatId, {
      ...
      bet,
      expires: Date.now() + 60000, // 60 сек на принятие
    });
  ...
  bot.action('duel_accept', async (ctx) => {
    ...
    await ctx.editMessageText(
      ...
      `${DUEL_PHRASES[Math.floor(Math.random() * DUEL_PHRASES.length)]}\n\n` +
  ...
  bot.command(['guess', 'угадай'], async (ctx) => {
    ...
    if (!num || isNaN(num)) {
      ...
      guessGames.set(key, { number: secret, attempts: 5 });
      return ctx.reply(
        `🔢 <b>Угадай число!</b>\n\n` +
    ...
    if (game.attempts <= 0) {
      guessGames.delete(key);
      return ctx.reply(
        `😔 Попытки закончились! Загаданное число было: <b>${game.number}</b>\n\nНачни заново: /guess`,
        { parse_mode: 'HTML' }
      );
    }
