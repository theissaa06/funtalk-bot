...
// Если старый неудачный файл есть — не важно, создаём новый фикс
...
if (!code.includes(insertBefore)) {
  console.error("❌ Не нашёл bot.on('message', async (msg) => {");
  process.exit(1);
}
...
`;
  ...
  code = code.replace(insertBefore, menuBlock + "\n" + insertBefore);
}
...
`;
  ...
  const insertAfterCallbackStart = callbackMarker;
  code = code.replace(insertAfterCallbackStart, insertAfterCallbackStart + topicCallback);
}

fs.writeFileSync(path, code, "utf8");

console.log("✅ ЛС-меню как у Iris добавлено");
console.log("✅ Вставка сделана перед bot.on('message'), а не перед SCHEDULERS");
